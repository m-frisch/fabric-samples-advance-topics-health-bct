...
approveForMyOrg() {
  ...
  setGlobals $ORG
  set -x
  peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.bct.mat --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} --version ${CC_VERSION} --package-id ${PACKAGE_ID} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
commitChaincodeDefinition() {
  ...
  set -x
  peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.bct.mat --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name ${CC_NAME} $PEER_CONN_PARMS --version ${CC_VERSION} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
chaincodeInvokeInit() {
  ...
  peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.bct.mat --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS --isInit -c ${fcn_call} >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
...
if [[ -z "$ARG3" ]]; then
  ...
  ## package the chaincode
  packageChaincode $ARG3

  ## Install chaincode on peer0.org1 and peer0.org2
  installChaincode $ARG3
  #installChaincode 1
  #installChaincode 2
  ...
  ## check whether the chaincode definition is ready to be committed
  ## expect org1 to have approved and org2 not to
  checkCommitReadiness $ARG3 "\"Org${ARG3}MSP\": false" "\"Org1MSP\": true" "\"Org2MSP\": true"
  checkCommitReadiness 1 "\"Org${ARG3}MSP\": false" "\"Org1MSP\": true" "\"Org2MSP\": true"
  checkCommitReadiness 2 "\"Org${ARG3}MSP\": false" "\"Org1MSP\": true" "\"Org2MSP\": true"

  ## now approve also for org2
  approveForMyOrg $ARG3

  ## check whether the chaincode definition is ready to be committed
  ## expect them both to have approved  
  checkCommitReadiness $ARG3 "\"Org${ARG3}MSP\": true" "\"Org1MSP\": true" "\"Org2MSP\": true"
  checkCommitReadiness 1 "\"Org${ARG3}MSP\": true" "\"Org1MSP\": true" "\"Org2MSP\": true"
  checkCommitReadiness 2 "\"Org${ARG3}MSP\": true" "\"Org1MSP\": true" "\"Org2MSP\": true"

  ## now that we know for sure both orgs have approved, commit the definition
  commitChaincodeDefinition $ARG3 1 2
