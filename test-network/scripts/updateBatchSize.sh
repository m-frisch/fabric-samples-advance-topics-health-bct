...
docker exec -e CHANNEL_NAME=mychannel cli sh -c 'peer channel fetch config config_block.pb -o orderer.bct.mat:7050 -c $CHANNEL_NAME --tls --cafile $ORDERER_CA'
...
CORE_PEER_TLS_ROOTCERT_FILE=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/bct.mat/orderers/orderer.bct.mat/tls/ca.crt
CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/bct.mat/users/Admin@bct.mat/msp
CORE_PEER_ADDRESS=orderer.bct.mat:7050

docker exec -e CORE_PEER_LOCALMSPID=$CORE_PEER_LOCALMSPID -e CORE_PEER_TLS_ROOTCERT_FILE=$CORE_PEER_TLS_ROOTCERT_FILE -e CORE_PEER_MSPCONFIGPATH=$CORE_PEER_MSPCONFIGPATH -e CORE_PEER_ADDRESS=$CORE_PEER_ADDRESS -e CHANNEL_NAME=mychannel  cli sh -c 'peer channel update -f final_update_in_envelope.pb -c $CHANNEL_NAME -o orderer.bct.mat:7050 --tls --cafile $ORDERER_CA'
