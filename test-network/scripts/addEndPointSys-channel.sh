ORDERER_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/bct.mat/orderers/orderer.bct.mat/msp/tlscacerts/tlsca.bct.mat-cert.pem
docker exec -e ORDERER_CA=$ORDERER_CA orderer-cli sh -c 'peer channel fetch config config_block.pb -o orderer.bct.mat:7050 -c system-channel --tls --cafile $ORDERER_CA'
...
docker exec orderer-cli sh -c 'jq ".channel_group.values.OrdererAddresses.value.addresses += [\"orderer2.bct.mat:7050\"]" config.json > modified_config.json'
...
docker exec -e ORDERER_CA=$ORDERER_CA orderer-cli sh -c 'peer channel update -f config_update_in_envelope.pb -c system-channel -o orderer.bct.mat:7050 --tls true --cafile $ORDERER_CA'
