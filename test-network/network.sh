...
function createConsortium() {
  ...
  set -x
  configtxgen -profile $CONSORTIUM_PROFILE -channelID system-channel -outputBlock ./system-genesis-block/genesis.block
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
...
## define orderer consortium profile
if [ -z "$CONSORTIUM_PROFILE" ]; then
  export CONSORTIUM_PROFILE="TwoOrgsOrdererGenesis"
fi
