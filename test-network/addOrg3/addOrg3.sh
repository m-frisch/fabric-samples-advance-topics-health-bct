...
function generateOrg3Definition() {
  ...
  echo "##########################################################"
   export FABRIC_CFG_PATH=$PWD
   set -x
   configtxgen -printOrg Org3MSP > ../organizations/peerOrganizations/org3.bct.mat/org3.json
   res=$?
   { set +x; } 2>/dev/null
   if [ $res -ne 0 ]; then
...
function addOrg3 () {
  ...
  if [ ! -d "../organizations/peerOrganizations/org3.bct.mat" ]; then
    generateOrg3
  ...
  echo "###############################################################"
  docker exec Org3cli ./scripts/org3-scripts/updateChannelConfig.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
  ...
  echo "###############################################################"
  docker exec Org3cli ./scripts/org3-scripts/joinChannel.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
