...
function generateOrg4Definition() {
  ...
  echo "##########################################################"
   export FABRIC_CFG_PATH=$PWD
   set -x
   configtxgen -printOrg Org4MSP > ../organizations/peerOrganizations/org4.bct.mat/org4.json
   res=$?
   { set +x; } 2>/dev/null
   if [ $res -ne 0 ]; then
...
function addOrg4 () {
  ...
  if [ ! -d "../organizations/peerOrganizations/org4.bct.mat" ]; then
    generateOrg4
  ...
  echo "###############################################################"
  docker exec Org4cli ./scripts/org4-scripts/updateChannelConfig.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
  ...
  echo "###############################################################"
  docker exec Org4cli ./scripts/org4-scripts/joinChannel.sh $CHANNEL_NAME $CLI_DELAY $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
